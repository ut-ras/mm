...
void app_main1() {
  ...
  init_distance_sensor(&left, LEFT_FRONT_PIN, LEFT_SIDE_PIN,
                           LEFT_EMITTER);

  init_distance_sensor(&right, RIGHT_FRONT_PIN, RIGHT_SIDE_PIN,
                           RIGHT_EMITTER);
  ...
  while(true) {
    ...
    printf("Left: %d\nLeftF: %d\nRight: %d\nRightF: %d\n", leftVals[1], leftVals[0], rightVals[1], rightVals[0]);
